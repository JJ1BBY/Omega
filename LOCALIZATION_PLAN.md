...
## 発見済みの新たな抽出漏れカテゴリ: strcpy/strcat経由の文字列(未対応)

プレイテスト中に「アリーナの戦闘メッセージが英語のまま」というバグ報告が
あり調査した結果、元の機械的抽出(`print1/print2/print3/nprint1/nprint2/
nprint3/mprint`への**直接のリテラル引数**のみを対象にしていた)が見逃す、
系統的な抜け漏れパターンを発見した:

```c
/* aux2.c の例。リテラルはまず strcpy/strcat でバッファ(Str1など)に
   組み立てられ、その変数だけが mprint() に渡されるため、
   「print系関数へのリテラル引数」を機械的に拾う抽出では検出されない */
strcpy(Str1,"You punch ");
strcat(Str1,actionlocstr(Player.meleestr[i+1]));
mprint(Str1);
```

`aux2.c`は`72f5658`("Fix aux2.c combat messages missed by the original
extraction")で対応済み(6関数、`IDS_MSG_22739`〜`22770`)。

**未対応**: 同じ`strcpy(StrN,"...")` / `strcat(StrN,"...")`パターンを
grepで探すと、**約27ファイルに数百件規模**で存在することを確認済み
(概算、`aux1.c`・`mtalk.c`・`gen1.c`・`mon.c`・`etc.c`・`mmelee.c`・
`aux3.c`が特に多い)。これらは今後同様の「英語のまま表示される」バグ
報告の温床になる可能性が高い。対応時は各ファイルごとに:
1. `strcpy(StrN,"...")` / `strcat(StrN,"...")`のリテラルを洗い出す
2. `resource.h`・`Strings.en.rc`・`Strings.ja.rc`に`IDS_MSG_*`を追加
   (次の空き番号から採番。現在の最大は`22770`)
3. ソース側を`LS(id)`呼び出しに置き換え
4. 複数の分岐が同じ`strcat`呼び出し(共有サフィックス)に合流する場合、
   個々の分岐で異なる動詞を使い分けられない(SOV語順を無理に再現しようと
   すると破綻する)。`aux2.c`で採用した回避策: 動詞側を先に完結させ
   (「〜した、」「〜した。」)、対象(モンスター名など、英語のまま)は
   その後に続ける「2つの短い節」方式(例:「殴りつけた、goblin！」)。
   日本語のゲームログでは違和感のない表現。

**要注意**: `minit.h`のモンスター名テーブル(`Monsters[]`、150体、表示名+
死体名で約300件)は、`sitenames[]`/`spell_names[]`と同様のリスクがある
ことを確認済み — `mtalk.c:269`に`strcmp(m->monstring,"itinerant merchant")`
という、モンスター名を直接文字列比較でロジック分岐に使っている箇所が
実在する。モンスター名を翻訳する場合は、この種の`strcmp`箇所を全て
洗い出してから着手すること(⑤の項目と同様、単純な一括置換は危険)。
