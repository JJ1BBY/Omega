...
// _WIN32	    test_file_access(Str1, 'w') == 0)
	    test_file_access(required_file_list[file], 'w') == 0)
	{
	    impossible = TRUE;
// _WIN32	    printf("\nError! File not appendable or accessible: %s", Str1);
	    printf("\nError! File not appendable or accessible: %s", required_file_list[file]);
	}
